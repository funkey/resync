...
def read_lines_document(fs, doc_uid, page_uid):
    ...
    try:
        content = fs.read_file(os.path.join(doc_uid, page_uid + ".rm"), binary=True)
...
class Lines:
    ...
    def _read_from(self, source):
        ...
        if version == 3:
            ...
            raise UnsupportedVersion(
                f"Enountered invalid version {version}, only 3 or 5 are supported"
            )
    ...
    def _read_segment(self, source):
        x, y, speed, direction, width, pressure = read_struct(S_SEGMENT, source)
