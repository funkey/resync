...
def flat_pencil_width(segment):
    return (round(segment.width * 0.55, 2), round(segment.pressure, 2))
...
def bezierInterpolation(K, coord):
    ...
    b[0] = 2.0
    c[0] = 1.0
    r[0] = K[0][coord] + 2.0 * K[1][coord]

    for i in range(1, n - 1):
        a[i] = 1.0
        b[i] = 4.0
        c[i] = 1.0
        r[i] = 4.0 * K[i][coord] + 2.0 * K[i + 1][coord]

    a[n - 1] = 2.0
    b[n - 1] = 7.0
    c[n - 1] = 0.0
    r[n - 1] = 8.0 * K[n - 1][coord] + K[n][coord]
        ...
        for li, l in enumerate(page.layers):
            ...
            for k in l.strokes:
                ...
                if k.pen == 8:
                    ...
                    logger.debug("A: %f", time.perf_counter() - T1)
                    T1 = time.perf_counter()
                    subarea = subarea.simplified()  # this is expensive
                    logger.debug("B: %f", time.perf_counter() - T1)
                    T1 = time.perf_counter()
                    ...
                    if (simplify > 0 or smoothen) and tool == FINELINER_TOOL:
                        ...
                        for (w, p), segments in groupby(k.segments[1:], calcwidth):
                            ...
                            if pencil_resolution and tool == PENCIL_TOOL and p:
                                ...
                                item = QGraphicsPathItem(path, group)
                                pen.setBrush(
                                    pencilBrushes().getBrush(
                                        int(p * 0.7), scale=pencil_resolution
                                    )
                                )
